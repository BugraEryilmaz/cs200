...
/**
 * This interface describes the cs200 specific launch attributes
 * (which are not part of the Debug Adapter Protocol).
 * The schema for these attributes lives in the package.json of the cs200 extension.
...
export class MockDebugSession extends LoggingDebugSession {
  ...
  protected async launchRequest(
    ...
    WebViewPanel.addCallback((data: any) => {
      switch (data.type) {
        case "terminal-input": {
          this._terminal?.write(data.value);
          break;
        }
        case "onInfo": {
          if (!data.value) {
